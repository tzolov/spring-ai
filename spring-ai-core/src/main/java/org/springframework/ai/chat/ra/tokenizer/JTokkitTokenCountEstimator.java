...
import org.springframework.util.CollectionUtils;
 ...
 public class JTokkitTokenCountEstimator implements TokenCountEstimator {
	...
	@Override
	public int estimate(Document document) {
		return this.estimator.countTokens(document.getContent());
	}

	@Override
	public int estimateDocuments(Iterable<Document> documents) {
		int totalSize = 0;
		for (Document doc : documents) {
			totalSize += this.estimate(doc);
		}
		return totalSize;
   }
