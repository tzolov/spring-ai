 ...
 * @since 1.0.0
 */
public abstract class AbstractObservableChatModel implements ChatModel {
	...
	public AbstractObservableChatModel(ObservationRegistry observationRegistry) {
	...
	public ChatResponse call(Prompt prompt) {
		...
		observationContext.setChatModelName(this.getClass().getSimpleName());

		return ChatModelObservationDocumentation.AI_CHAT_MODEL
			.observation(this.observationConvention, DEFAULT_OBSERVATION_CONVENTION, () -> observationContext,
					this.observationRegistry)
			.observe(() -> {
				ChatResponse chatResponse = this.doCall(prompt);
	...
	public Flux<ChatResponse> stream(Prompt prompt) {
		...
		return Flux.defer(() -> {
			...
			return this.doStream(prompt).doOnSubscribe(subscription -> {
	...
	abstract public ChatResponse doCall(Prompt prompt);

	abstract public Flux<ChatResponse> doStream(Prompt prompt);
