...
import org.springframework.ai.chat.memory.session.compaction.CompactionResult;
import org.springframework.ai.chat.memory.session.compaction.CompactionStrategy;
import org.springframework.ai.chat.memory.session.compaction.CompactionTrigger;
...
public interface SessionService {
	 ...
	 * {@link org.springframework.ai.chat.memory.session.compaction.CompactionRequest},
	 * checks the trigger, and — only if the trigger fires — runs the strategy and writes
	 * the compacted list back to the repository. No-ops (trigger does not fire, or
	 * strategy archives nothing) skip the repository write entirely.
	 * <p>
	 * Pass {@code (req) -> true} as the trigger to compact unconditionally.
	 * @return the compaction result;
	 * {@link org.springframework.ai.chat.memory.session.compaction.CompactionResult#archivedEvents()}
