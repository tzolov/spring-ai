...
=== STDIO and SSE Server Configuration Properties
...
=== Streamable-HTTP Server (WebFlux and WebMVC) Configuration Properties
...
NOTE: Stateless servers doesn't support messages requests to the MCP client (e.g. elicitation, sampling, ping)
...
=== Stateless Server Configuration Properties
...
The MCP Server supports multiple transport mechanisms, each with its dedicated starter:
...
* Streamable Spring MVC - `spring-ai-starter-mcp-server-streamable-webmvc`
* Streamable Spring WebFlux - `spring-ai-starter-mcp-server-streamable-webflux`
* Stateless Spring MVC - `spring-ai-starter-mcp-server-stateless-webmvc`
* Stateless Spring WebFlux - `spring-ai-starter-mcp-server-stateless-webflux`
...
NOTE: Not applicable for Stateless servers.
...
TIP: For Stateless servers use the `McpStatelessServerFeatures.SyncResourceSpecification` (or `McpStatelessServerFeatures.AsyncResourceSpecification`) class instead. Instead of `exchange` the handler will be provided with `context` of type `McpTransportContext`.
...
TIP: For Stateless servers use the `McpStatelessServerFeatures.SyncPromptSpecification` (or `McpStatelessServerFeatures.AsyncPromptSpecification`) class instead. Instead of `exchange` the handler will be provided with `context` of type `McpTransportContext`.
...
TIP: For Stateless servers use the `McpStatelessServerFeatures.SyncCompletionSpecification` (or `McpStatelessServerFeatures.AsyncCompletionSpecification`) class instead. Instead of `exchange` the handler will be provided with `context` of type `McpTransportContext`.
