...
| spring.ai.azure.openai.embedding.options.input-type | specifies the input type to use for embedding search. | -
| spring.ai.azure.openai.embedding.options.dimensions | The number of dimensions the resulting output embeddings should have. Only supported in `text-embedding-3` and later models. | -
|====
