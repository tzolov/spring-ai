...
import org.springframework.ai.mcp.server.streamable.autoconfigure.McpStreamableServerAutoConfiguration;
import org.springframework.ai.mcp.server.streamable.autoconfigure.McpStreamableServerProperties;
import org.springframework.ai.mcp.server.streamable.autoconfigure.ToolCallbackConverterAutoConfiguration;
...
public class McpStreamableServerAutoConfigurationIT {
	...
	void asyncConfiguration() {
		this.contextRunner
			.withPropertyValues("spring.ai.mcp.server.streamable.type=ASYNC",
					"spring.ai.mcp.server.streamable.name=test-server", "spring.ai.mcp.server.streamable.version=2.0.0",
					"spring.ai.mcp.server.streamable.instructions=My MCP Server",
					"spring.ai.mcp.server.streamable.request-timeout=30s")
			.run(context -> {
	...
	void syncServerInstructionsConfiguration() {
		this.contextRunner.withPropertyValues("spring.ai.mcp.server.streamable.instructions=Sync Server Instructions")
			.run(context -> {
				McpStreamableServerProperties properties = context.getBean(McpStreamableServerProperties.class);
	...
	void serverNotificationConfiguration() {
		this.contextRunner
			.withPropertyValues("spring.ai.mcp.server.streamable.tool-change-notification=false",
					"spring.ai.mcp.server.streamable.resource-change-notification=false")
			.run(context -> {
				McpStreamableServerProperties properties = context.getBean(McpStreamableServerProperties.class);
	...
	void disabledConfiguration() {
		this.contextRunner.withPropertyValues("spring.ai.mcp.server.streamable.enabled=false").run(context -> {
			assertThat(context).doesNotHaveBean(McpSyncServer.class);
	...
	void notificationConfiguration() {
		this.contextRunner
			.withPropertyValues("spring.ai.mcp.server.streamable.tool-change-notification=false",
					"spring.ai.mcp.server.streamable.resource-change-notification=false",
					"spring.ai.mcp.server.streamable.prompt-change-notification=false")
			.run(context -> {
				McpStreamableServerProperties properties = context.getBean(McpStreamableServerProperties.class);
				assertThat(properties.isToolChangeNotification()).isFalse();
				assertThat(properties.isResourceChangeNotification()).isFalse();
	...
	void asyncToolSpecificationConfiguration() {
		this.contextRunner.withPropertyValues("spring.ai.mcp.server.streamable.type=ASYNC")
	...
	void capabilitiesConfiguration() {
		this.contextRunner
			.withPropertyValues("spring.ai.mcp.server.streamable.capabilities.tool=false",
					"spring.ai.mcp.server.streamable.capabilities.resource=false",
					"spring.ai.mcp.server.streamable.capabilities.prompt=false",
					"spring.ai.mcp.server.streamable.capabilities.completion=false")
			.run(context -> {
				McpStreamableServerProperties properties = context.getBean(McpStreamableServerProperties.class);
	...
	void toolResponseMimeTypeConfiguration() {
		this.contextRunner
			.withPropertyValues("spring.ai.mcp.server.streamable.tool-response-mime-type.test-tool=application/json")
			.withUserConfiguration(TestToolConfiguration.class)
			.run(context -> {
				McpStreamableServerProperties properties = context.getBean(McpStreamableServerProperties.class);
	...
	void requestTimeoutConfiguration() {
		this.contextRunner.withPropertyValues("spring.ai.mcp.server.streamable.request-timeout=45s").run(context -> {
			McpStreamableServerProperties properties = context.getBean(McpStreamableServerProperties.class);
	...
	void endpointConfiguration() {
		this.contextRunner.withPropertyValues("spring.ai.mcp.server.streamable.mcp-endpoint=/my-mcp").run(context -> {
			McpStreamableServerProperties properties = context.getBean(McpStreamableServerProperties.class);
